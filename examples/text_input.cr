...
class TextField < Gosu::TextInput
  ...
  def filter(new_text)
    allowed_length = [LENGTH_LIMIT - text.size, 0].max
  ...
  def draw(z)
    ...
    pos_x = x + FONT.text_width(self.text[0...self.caret_pos.to_i])
    sel_x = x + FONT.text_width(self.text[0...self.selection_start.to_i])
    sel_w = pos_x - sel_x
  ...
  def under_mouse?
    @window.mouse_x > x - PADDING && @window.mouse_x < x + WIDTH + PADDING &&
      @window.mouse_y > y - PADDING && @window.mouse_y < y + height + PADDING
  end

  # Tries to move the caret to the position specifies by mouse_x
  def move_caret_to_mouse
    # Test character by character
    1.upto(self.text.size) do |i|
      if @window.mouse_x < x + FONT.text_width(text[0...i])
        self.selection_start = i - 1
        self.caret_pos = i - 1
        return
      end
    end
    # Default case: user must have clicked the right edge
    self.selection_start = self.text.size
    self.caret_pos = self.text.size
  end
end
...
class TextInputDemo < Gosu::Window
  ...
  def initialize
    ...
    @text_fields = [] of TextField
    3.times { |index| @text_fields << TextField.new(self, 50, 300 + index * 50) }
  ...
  def button_down(id)
    if id == Gosu::KB_TAB
      ...
      self.text_input.as(TextField).move_caret_to_mouse unless self.text_input.nil?
    else
      super
    end
  end
